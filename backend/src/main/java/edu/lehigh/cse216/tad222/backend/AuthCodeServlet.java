...
public class AuthCodeServlet extends AbstractAppEngineAuthorizationCodeServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        StringBuilder oauthUrl = new StringBuilder().append("https://accounts.google.com/o/oauth2/auth")
                .append("?client_id=").append(request.getPart("clientid")) // the client id from the api console
                                                                           // registration
                .append("&response_type=code").append("&scope=openid%20email") // scope is the api permissions we are
                                                                               // requesting
                .append("&redirect_uri=" + Util.SITE + "/messages") // the servlet that google redirects to after
                                                               // authorization
    ...
    protected AuthorizationCodeFlow initializeFlow() throws IOException {
        return new AuthorizationCodeFlow.Builder(BearerToken.authorizationHeaderAccessMethod(), new NetHttpTransport(),
                new JacksonFactory(), new GenericUrl(Util.SITE + "/messages"),
                new BasicAuthentication(Util.getClientId(), Util.getClientSecret()), Util.getClientId(),
                Util.SITE + "/login").setCredentialDataStore(StoredCredential
